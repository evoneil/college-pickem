...
function CurrentWeekPicks() {
  ...
  const fetchData = async () => {
    ...
    if (picksData) {
      ...
      // Check using current local state to avoid clobbering unsaved changes
      const localUnchanged =
        JSON.stringify(draftPicks) === JSON.stringify(originalPicks)
          ...
          {games.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-6">
              {games.map((game) => {
                ...
                return (
                  <div key={game.id} className={clsx('bg-[#14121C] border border-[#484454] rounded-xl p-4 space-y-2', isCorrectPick && 'bg-[radial-gradient(ellipse_70%_50%_at_50%_-0%,_#1E4A31_0%,_#14121C_100%)]', isWrongPick && 'bg-[radial-gradient(ellipse_70%_50%_at_50%_-0%,_#4A1E1E_0%,_#14121C_100%)]')}>
